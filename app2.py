...
# GROQ API KEY
# ==================================================
GROQ_API_KEY = st.secrets.get("GROQ_API_KEY", "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")
...
if pdf_file:
    ...
    if st.button("🚀 Convert to Excel", use_container_width=True, type="primary"):
        try:
            ...
            st.error(f"❌ Groq API error: {e.response.status_code} — check your API key in secrets.")
        except Exception as e:
            st.error(f"❌ Error: {e}")
